...
class Solver
{
  ...
  //Iterative solution
  void SOLVE(Array& pp, Array& ff, Array& U, Array& V, Array& F, Array& G,Data& mod,ParticleLine& Part, double xlength, double s_x1, double s_y1,double s_x2,double s_y2,unsigned int n_part, double t_vis, double del_save, double del_inj);
...
void Solver::SOLVE(Array& pp, Array& ff, Array& U, Array& V, Array& F, Array& G, Data& dom,ParticleLine& Part, double xlength,double s_x1,double s_y1,double s_x2,double s_y2,unsigned int n_part, double t_vis, double del_save, double del_inj){
  ...
  if((t-tmp1)>=del_inj){
    SL.push_back(StreakLine(s_x1,s_y1,s_x2,s_y2,xmax,ymax,n_part));
    tmp1=t;
  }
  ...
  if((t-tmp2)>=del_save && t>t_vis){
    ...
    for(size_t ii=0;ii<SL.size();++ii){
      ...
      for(size_t jj=0; jj<n_part;++jj){
	if(floor(pp[jj].X+0.5)<xlength)
